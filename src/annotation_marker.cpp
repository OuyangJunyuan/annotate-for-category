...
namespace annotate {
    ...
    AnnotationMarker::AnnotationMarker(AnnotateDisplay *annotate_display,
                                       const shared_ptr<InteractiveMarkerServer> &server,
    ...
    void AnnotationMarker::updateMenu(const PointContext &context) {
        ...
        handle = menu_handler_.insert(mode_menu, "Rotate",
                                      boost::bind(&AnnotationMarker::enableRotationControl, this, _1));
        menu_handler_.setCheckState(handle, mode_ == Rotate ? MenuHandler::CHECKED : MenuHandler::NO_CHECKBOX);
        handle = menu_handler_.insert(mode_menu, "Resize",
                                      boost::bind(&AnnotationMarker::enableResizeControl, this, _1));
        ...
        if (!label_keys_.empty()) {
            ...
            for (auto const &label : label_keys_) {
                auto const handle = menu_handler_.insert(label_menu, label,
                                                         boost::bind(&AnnotationMarker::setLabel, this, _1));
        ...
        if (!tag_keys_.empty()) {
            ...
            for (auto const &tag : tag_keys_) {
                auto const handle = menu_handler_.insert(tags_menu, tag,
                                                         boost::bind(&AnnotationMarker::setTag, this, _1));
    ...
    void AnnotationMarker::saveMove() {
        bool const merge_with_previous =
                !undo_stack_.empty() && undo_stack_.top().undo_description == "move" &&
                undo_stack_.top().time.elapsed() < 800;
    ...
    AnnotationMarker::PointContext AnnotationMarker::analyzePoints() const {
        ...
        if (transform_listener.canTransform("current_annotation", cloud->header.frame_id, time, &error)) {
            ...
            Vector3 points_min(numeric_limits<float>::max(), numeric_limits<float>::max(),
                               numeric_limits<float>::max());
            Vector3 points_max(numeric_limits<float>::min(), numeric_limits<float>::min(),
    ...
    void AnnotationMarker::commit() {
        ...
        track_.erase(
                remove_if(track_.begin(), track_.end(),
        ...
        */
        ...
        //获取数据集文件路径
        QFile pts_file;
        QString pts_dir_path = annotate_display_->dataset_root_path_ + "/" + cate_dir.c_str() + "/points";
        QString pts_file_path =
                pts_dir_path + "/" + to_string(annotate_display_->num_labels_[label_]).c_str() + ".pts";

        MKDIR(pts_dir_path);// 不存在目录就创建目录

        if (pts_file.exists(pts_file_path)) { // 存在文件就删除重写
            pts_file.remove(pts_file_path);
        }
        ...
        if (transform_listener.canTransform("current_annotation", cloud->header.frame_id, time, &error)) {
            ...
            pcl::PointCloud<pcl::PointXYZ>::Ptr annotation_cloud(new pcl::PointCloud<pcl::PointXYZ>),
                    box_cloud(new pcl::PointCloud<pcl::PointXYZ>);
            pcl_ros::transformPointCloud(pointcloud, *annotation_cloud, trafo);
            ...
            if (!marker_.controls.empty() && !marker_.controls.front().markers.empty()) {
                ...
                box_filter.setMin(Eigen::Vector4f(box_min.x(), box_min.y(), box_min.z(), 1.0));
                box_filter.setMax(Eigen::Vector4f(box_max.x(), box_max.y(), box_max.z(), 1.0));
                box_filter.setNegative(false);//保留盒子内的点
                box_filter.setInputCloud(annotation_cloud);
            ...
            Eigen::Affine3f centralization = Eigen::Affine3f::Identity();
    ...
    void AnnotationMarker::setTime(const ros::Time &time) {
        ...
        if (!track_.empty()) {
            auto const prune_before_track_start =
                    time < track_.front().center.stamp_ && track_.front().timeTo(time) > 1.0;
        ...
        sort(track.begin(), track.end(),
             [time](TrackInstance const &a, TrackInstance const &b) -> bool {
                 return a.timeTo(time) < b.timeTo(time);
             });
